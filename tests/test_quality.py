...
class TestWidgetQualityAndConstraints(unittest.TestCase):
    ...
    def get_javascript_blocks(self, html_content):
        ...
        return re.findall(r'<script\b[^>]*>(.*?)</script\b[^>]*>', html_content, flags=re.DOTALL | re.IGNORECASE)
    ...
    def test_es5_conformance(self):
        ...
        for i, js in enumerate(js_blocks, 1):
            ...
            if re.search(r'\bconst\b', clean):
                self.fail(f"JavaScript block #{i} in src/widget.html contains ES6 'const' keyword! Please use 'var' instead.")
            if re.search(r'\blet\b', clean):
                self.fail(f"JavaScript block #{i} in src/widget.html contains ES6 'let' keyword! Please use 'var' instead.")
            
            # 2. Check for arrow functions '=>'
            if '=>' in clean:
                self.fail(f"JavaScript block #{i} in src/widget.html contains ES6 arrow function '=>'! Please use standard ES5 'function()' syntax.")
                
            # 3. Check for template literals (backticks)
            if '`' in clean:
                self.fail(f"JavaScript block #{i} in src/widget.html contains ES6 template literals (backticks)! Please use single or double quotes.")
            ...
            if re.search(r'\?\.(?!\d)', clean):
                self.fail(f"JavaScript block #{i} in src/widget.html contains ES2020 optional chaining '?.'! Please use standard conditional checks.")
                
            # 5. Check for ES6 class declaration
            if re.search(r'\bclass\b', clean):
                self.fail(f"JavaScript block #{i} in src/widget.html contains ES6 'class' keyword! Please use prototype-based objects.")
            ...
            if '...' in clean:
                self.fail(f"JavaScript block #{i} in src/widget.html contains ES6 spread/rest operator '...'!")

    def test_no_style_tags(self):
        """Verify that src/widget.html does not contain any <style> tags."""
        source_code = self.read_file_content('src/widget.html')
        self.assertNotIn('<style', source_code.lower(), "src/widget.html must not contain '<style>' tags. All styles must be inline.")
        self.assertNotIn('</style>', source_code.lower(), "src/widget.html must not contain '</style>' tags. All styles must be inline.")
        ...
        for rel_path in files_to_check:
            ...
            for i, js in enumerate(js_blocks, 1):
                ...
                if re.search(r'\.innerHTML\s*=|\[[\'"]innerHTML[\'"]\]\s*=', clean):
                    self.fail(f"JavaScript block #{i} in {rel_path} assigns to dangerous DOM sink 'innerHTML'! Use textContent, DOMParser, or replaceChildren instead to prevent XSS.")
                if re.search(r'\.outerHTML\s*=|\[[\'"]outerHTML[\'"]\]\s*=', clean):
                    self.fail(f"JavaScript block #{i} in {rel_path} assigns to dangerous DOM sink 'outerHTML'! Use safe elements builder APIs instead to prevent XSS.")
                if 'document.write' in clean:
                    self.fail(f"JavaScript block #{i} in {rel_path} calls 'document.write'! This is a dangerous DOM sink.")
                if 'insertAdjacentHTML' in clean:
                    self.fail(f"JavaScript block #{i} in {rel_path} calls 'insertAdjacentHTML'! Use textContent or DOMParser instead.")
                ...
                for dialogue in ['alert', 'confirm', 'prompt']:
                    ...
                    if re.search(r'\b' + dialogue + r'\b', clean):
                        self.fail(f"JavaScript block #{i} in {rel_path} uses blocking dialogue '{dialogue}'! Use non-blocking custom UI notification components instead.")
    ...
    def test_i18n_translation_keys(self):
        ...
        self.assertIsNotNone(i18n_match, "Could not find i18n object definition in src/widget.html")
        i18n_content = i18n_match.group(1)
        
        # Find all languages inside i18n (e.g. en: { ... })
        lang_blocks = re.findall(r'(\w+)\s*:\s*\{([^}]+)\}', i18n_content)
        self.assertTrue(len(lang_blocks) > 0, "No language blocks found in i18n definition")
        ...
        self.assertIn('en', lang_keys, "English (en) translation is missing from i18n dictionary")
        ...
        for lang, keys in lang_keys.items():
            ...
            self.assertEqual(len(missing_keys), 0, f"Language '{lang}' is missing translation keys: {missing_keys}")
            self.assertEqual(len(extra_keys), 0, f"Language '{lang}' has extra translation keys: {extra_keys}")
    ...
    def test_widget_size_budget(self):
        ...
        if not os.path.exists(minified_path):
            self.skipTest("dist/widget.min.html does not exist yet. Please run build.py first.")
            
        size = os.path.getsize(minified_path)
        # Limit to 64KB (65536 bytes)
        self.assertLess(size, 65536, f"Minified widget size ({size} bytes) exceeds the budget of 64KB!")
    ...
    def test_smiley_and_comment_regression_on_dist(self):
        ...
        for i, js in enumerate(js_blocks, 1):
            if re.search(r'(?<!https:)(?<!http:)\/\/', js):
                self.fail(f"Found single-line JS comment '//' in dist/widget.min.html script block #{i}!")
                
            # Verify no smiley triggers
            self.assertNotIn('})', js, f"Found smiley trigger '}})' in dist/widget.min.html script block #{i}!")
            self.assertNotIn('8)', js, f"Found smiley trigger '8)' in dist/widget.min.html script block #{i}!")
