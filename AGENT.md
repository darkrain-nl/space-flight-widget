...
5. **Local Verification**: Do not open `index.html` as a local `file://` URI in the browser because the browser's sandbox/CSP may block API requests. Instead, spin up a local web server (e.g., `python3 -m http.server 8090`) and navigate to `http://10.0.10.191:8090/index.html` to test.
